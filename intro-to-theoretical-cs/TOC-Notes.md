...
⚠️ on peut uniquement modifier l'input, puis donner cette instance modifiée à notre autre problème. on ne peut pas, sans oracle, utiliser le résultat du P2 pour P1 (comme la machine qui si une machine ne loop pas, plutôt que halt)
c'est la différence entre **mapping reduction** et **turing reduction** (agir sur l'output)
##### Rice's theorem
...
⚠️ vérifier que c'est pas une question de propriétés de syntaxe (p. ex. le nb de states)
...
--> pour chaque entier, on peut vérifier p. ex. s'il est de la forme $2^x 3^y$ et si c'est le cas, le tester. 

---- 

#### Time complexity

$f(n)$ --> le nb de steps max que met $f$ pour n'importe quel input de taille $n$.

complexity class --> au moins une machien le fait dans ce temps

**tractable** $$ P = union_(k in NN) "Time" (n^k) $$

NTIME : classes d'équivalence pour ceux qui peuent e6Ter resolu avec une NRM
$$"NP" = union_(k in NN) "Utime"(n^k)$$

NP-hard : plus dur que tous les pb dans NP (donc potentiellement même pas NP, p. ex. si pas solvable en temps poly par un non deterministic TM ou RM)

NP-complete : dans NP et NP-hard

![[image-38.png]]

on construit une formule booléenne pour voir si c'est dans le langage

$$(C_{00} \land \rho_{\text{init}} \land \chi_{\text{one}} \land \bigwedge_t \chi_t \land \alpha)$$

$C_(0 0)$ : vérifier qu'au début on exécute la première instruction
$Chi_"one"$ : vérifier qu'on exécute qu'une seule instruction à chaque fois
$X_t$ : on a exécuté toutes les ops dans l'ordre

$P^(N P)$: nb poly de pb np à résoudre

NP : trouver une solution, une instance qui marche $exists$
CoNP : vérifier que toutes les solutions marchent 

![[image-39.png]]

maybe exists : fait un fork et après join regarde si au moins un
maybe forall : fork + tout

---- 

from scheduling to color --> on veut montrer que le scheduling n'est "rien d'autre" que color.

## Lambda calculus

**TRUE** $lambda x.lambda y.x$ (renvoie toujours la première valeur)
```python
if True:
	print("oui")
else:
...
**IFELSE**: $lambda b.lambda x.lambda y. b x y$

Le premier argument qu'on prend c'est $B$ qui va lui-même être une fonction **TRUE** ou **FALSE** et qui va nous permettre de choisir entre le membre de gauche x ou de droite y.

![[image-40.png]]
---

![[image-41.png]]
il nous manque des types
...
L’**η‑réduction** (ou **η‑équivalence**) : deux fonctions sont équivalentes si elles « se comportent pareil » sur tous les arguments. même si elles se réduisent pas à la même expression, si elles "font" la même chose, elles sont n-équivalente.

![[image-45.png]]

`Y = λf . (λx. f (x x))(λx. f (x x))` (f)
`Y = (λx. f (x x))(λx. f (x x))`
...
Let us recall the abstraction rule :
$$ (x : sigma, Gamma tack.r t : tau) / (Gamma tack.r (lambda x : sigma . t) : sigma -> tau) (arrow_I) $$
and the elimination rule :
$$ (Gamma tack.r t : sigma -> tau "       " Gamma tack.r u : sigma) / (Gamma tack.r (t u : tau)) (arrow_E) $$
--- 

$Gamma = {x: "nat", f: "nat" -> "nat"}$.

$$(f : ("nat" -> "nat"), Gamma tack.r f: ("nat" -> "nat") "      " x : "nat", Gamma tack.r x: "nat")/(Gamma tack.r f x : "nat") (arrow_E)$$

$$ (Gamma tack.r f x : "nat" "      " Gamma tack.r f : ("nat" -> "nat"))/(Gamma tack.r f(f x) : "nat") (arrow_E)$$

$$ (Gamma tack.r f x : "nat" "      " Gamma tack.r f : ("nat" -> "nat"))/(Gamma tack.r f(f x) : "nat") (arrow_E)$$

$$  (x: "nat", Gamma tack.r f(f x) : "nat") / (Gamma tack.r (lambda x. f(f x)): ("nat" -> "nat")) (arrow_I) $$

$$ (f : ("nat" -> "nat"), Gamma tack.r (lambda x:"nat". f(f x)) : ("nat" -> "nat")) / (Gamma tack.r (lambda f: ("nat" -> "nat").lambda x:"nat". f(f x)): (("nat" -> "nat") -> ("nat" -> "nat"))) (arrow_I)$$

![[image-46.png]]![[image-47.png]]
